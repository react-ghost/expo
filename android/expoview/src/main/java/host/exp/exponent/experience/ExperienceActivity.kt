...
open class ExperienceActivity : BaseExperienceActivity(), StartReactInstanceDelegate {
  ...
  fun setManifest(
    ...
    ExponentDB.saveExperience(this.manifestUrl, manifest, bundleUrl)
    ...
    detachSdkVersion = if (Constants.isStandaloneApp()) RNObject.UNVERSIONED else mSDKVersion
    if (RNObject.UNVERSIONED != mSDKVersion) {
      var isValidVersion = false
      for (version in Constants.SDK_VERSIONS_LIST) {
        if (version == mSDKVersion) {
          isValidVersion = true
          break
        }
      }
      if (!isValidVersion) {
        KernelProvider.instance.handleError(
          mSDKVersion + " is not a valid SDK version. Options are " +
    ...
    isCrashed = false
    Analytics.logEventWithManifestUrlSdkVersion(Analytics.LOAD_EXPERIENCE, manifestUrl, mSDKVersion)
    ...
    val finalNotificationObject = notificationObject
    ...
    runOnUiThread {
      ...
      if (isDebugModeEnabled) {
        notification = finalNotificationObject
        jsBundlePath = ""
        startReactInstance()
      } else {
        tempNotification = finalNotificationObject
  ...
  fun handleOptions(options: ExperienceOptions) {
    try {
      if (options.uri != null) {
        ...
        BranchManager.handleLink(this, options.uri, detachSdkVersion)
      }
