...
class Exponent private constructor(val context: Context, val application: Application) {
  ...
  private val bundleStrings: MutableMap<String, String> = mutableMapOf()

  fun getBundleSource(path: String): String? {
    synchronized(bundleStrings) {
      return if (bundleStrings.containsKey(path)) {
        val result = bundleStrings[path]
        bundleStrings.remove(path)
        result
      } else {
        null
      }
  ...
  fun loadJSBundle(
    ...
    shouldForceNetwork: Boolean = false
  ): Boolean {
    return loadJSBundle(
      manifest,
      urlString,
  ...
  fun loadJSBundle(
    ...
    val isDeveloping = manifest != null && manifest.isDevelopmentMode()
    if (isDeveloping) {
    ...
    try {
      val requestBuilder =
        if (KernelConstants.KERNEL_BUNDLE_ID == id) ExponentUrls.addExponentHeadersToUrl(urlString) else Request.Builder()
          .url(urlString)
        ...
        override fun onResponse(response: ExpoResponse) {
          ...
          try {
            ...
            if (response.networkResponse() == null || response.networkResponse()
              .code() == KernelConstants.HTTP_NOT_MODIFIED
            ) {
      ...
      if (shouldForceCache) {
        exponentNetwork.longTimeoutClient.tryForcedCachedResponse(
        ...
        exponentNetwork.longTimeoutClient.callSafe(request, callback)
      } else {
        exponentNetwork.longTimeoutClient.callDefaultCache(request, callback)
      }
    } catch (e: Exception) {
      bundleListener.onError(e)
    }
  ...
  companion object {
    ...
    @JvmStatic fun enableDeveloperSupport(
      ...
      if (debuggerHost.isNotEmpty() && mainModuleName.isNotEmpty()) {
        try {
